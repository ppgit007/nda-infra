locals {
  ...
  module_enabled = {
    ...
    redis             = lookup(local.deployment_flags, "redis", true)
  ...
  private_dns_zones = merge(
    ...
    local.module_enabled.private_endpoints && local.module_enabled.ai_search ? {
      ai_search = "privatelink.search.windows.net"
    } : {},
    local.module_enabled.private_endpoints && local.module_enabled.azure_openai ? {
  ...
  private_endpoint_definitions = local.module_enabled.private_endpoints ? merge(
    ...
    local.module_enabled.ai_search ? {
      ai_search = {
        name              = "${local.names.ai_search}-pe"
        target_id         = module.ai_search["enabled"].id
        subresource_names = ["searchService"]
        dns_zone_key      = "ai_search"
      }
    } : {},
    local.module_enabled.azure_openai ? {
      azure_openai = {
        name              = "${local.names.azure_openai}-pe"
        target_id         = module.azure_openai["enabled"].id
        subresource_names = ["account"]
        dns_zone_key      = "azure_openai"
      }
    } : {},
